...
⛔ (2026-09-18, D-056) 이동 「횟수」는 쓰지 않는다 — 1단계 승자가 lot 재추출 62.4%(D-033)이고 구현에도 걸린다(D-047).
그림 자체는 유효하다(세 값은 옳다).
...
def main():
    ...
    print("  그 하나가 끝까지 남은 병목이다 — 세 계열을 모두 써도 상호 오분류가 남았다.")
    print("  (「경계가 없어서 남는다」는 쓰지 않는다: 집계 수렴은 개별 경계 부재를 함의하지 않고")
    print("   판정할 W7은 미실행이다 — D-025·D-026 → D-056)")
