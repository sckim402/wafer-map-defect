...
검증 전략 — 라벨을 쓰지 않는 예측을 세워 확인한다:
  ...
  [1] lot 내 재현성  ★ 가장 결정적
      ...
      **같은 lot의 웨이퍼들이 비슷한 coverage를 가져야 한다.**
      Edge-Loc(국부·확률적 원인)은 재현성이 낮아야 한다.

      ⛔ 정정 (2026-09-17, D-052·D-053 → 12차 D-055): 원래 여기에 *"lot 정보는 라벨과
      완전히 독립이므로 재현성이 나오면 coverage는 물리를 잡고 있다"*고 썼다. **둘 다 틀렸다.**
      ⓐ 라벨 부여에 lot을 안 썼다는 것이지 **통계적 독립이 아니다** — 실측으로 9종 전부
         lot에 응집한다(`Edge-Ring` R=0.931). ⓑ lot 안 재현성은 **맵 shape(lot 안에서 거의 일정)·
         라벨 선별·수집 묶음**으로도 생기고 **공정(챔버) 원인과 구분되지 않는다.**
      ✅ 이 검사가 보이는 것은 **「lot 안 coverage 차이가 무작위 쌍보다 작다」는 관측**까지다.
...
def main():
    ...
    # ── [1] lot 내 재현성 ─────────────────────────────────
    print("=" * 72)
    print("[1] lot 내 재현성 — coverage가 lot 단위로 뭉치는가")
    print("=" * 72)
    print("  Edge-Ring 원인이 챔버 조건이면 같은 lot 웨이퍼끼리 비슷해야 한다 (필요조건일 뿐이다).\n")
    ...
    if rr < 0.85 and rr < rl:
        ...
        print("      lot 단위 원인 가설과 모순되지 않는다. ⛔ 챔버 원인의 확인이 아니다 —")
        print("      맵 shape·라벨 선별·수집 묶음과 구분되지 않는다 (D-053). 동어반복 판정은 [2][3]과 함께 본다")
    else:
