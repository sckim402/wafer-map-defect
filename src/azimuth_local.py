...
def arc_masks(theta, grid=False):
    ...
    **위반연속(upper semi-continuous)** — 경계에서 값이 양쪽 극한 이상이다.
    따라서 최대는 **중단점에서 달성**된다. **좌단 `{θⱼ − ARC}`만으로도 충분하다** —
    최대점 `s*`에서 **가장 가까운 왼쪽 좌단**까지 옮기면 그 사이에 좌단이 없으므로
    잃는 die가 없다(정수 격자 완전탐색 3,000판에서 미달 **0건**, D-053).
    여기서는 **합집합 `{θⱼ} ∪ {θⱼ − ARC}`(2n개)**를 쓴다 — **충분하고 옳지만 필요조건은 아니다.**

    ⛔ **정정 (10차 D-053 → 12차 D-055): 「좌단만 쓰면 틀린다」는 거짓이었다.**
    2026-09-16의 1차 구현(좌단만)이 **반사 불일치 25/300**을 낸 원인은 후보 집합이 아니라
    **부동소수 경계**다 — 시작점 바로 왼쪽의 미세한 음수가 `% 360`에서 360° 근처로 감겨 나간다.
    허용오차를 **오른쪽 경계에만** 주면 안 고쳐지고(검토자 재현 29/300) **양쪽에 주면 0/300**이다.
    당시 나는 그것을 *"먼 끝 die가 빠진다 → 좌단만으로는 최대를 놓친다"*로 **오진**했다.

    ⚠ **그래서 수치 허용오차는 이 함수의 본질이다.** `ARC_EPS`는 실제 각도 구조보다 훨씬 작아
    **경계 판정만 고친다.** **합집합 + `ARC_EPS`에서 반사 불일치 0/300**이다.
    """
    if grid:
